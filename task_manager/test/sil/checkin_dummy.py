...
import calendar
from frappe.utils import time_diff_in_hours, getdate, today, format_datetime, time_diff_in_hours
...
@frappe.whitelist(allow_guest=True)
def get_employee_details(session_user,from_date=None, to_date=None, specific_date=None):
    try:
        ...
        if from_date and to_date:
            ...
            try:
                ...
                if not session_user:
                    frappe.throw("Employee id not provided !!!")
                    frappe.log_error("Session user(session_user) not provided")
                ...
                # that is why we dont return anything or 'assign to' anything in the below line
